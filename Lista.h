...
template <class T> void Lista<T>::insertar_inicio(T dato) {
  ...
  Aux = cab->sig; // se guarda la direccion de memoria del primer nodo en el auxiliar
             // para no perderla
  cab->sig = new Nodo<T>; // la cabeza de la lista ahora apuntara hacia un nuevo nodo
  cab->sig->info = dato;  // el cual tendra como info el dato pasado como parametro
  cab->sig->sig = Aux; // y el nodo siguiente sera el que era la anterior cabeza de la lista

  tam++; // se aumenta el tamaño de la lista
}

// revisar, si hay 3 nodos y me piden insertar uno en la pos 5.
// debo decir que no se puede o
// añadir nodos suficientes para que mi nuevo nodo quede en el 5to lugar??
